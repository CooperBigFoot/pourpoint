...
fn execute_staged_seam_carve() {
    ...
    let seed_unit = search_session
        .catchments()
        .query_by_ids(&[seed_resolved.authority().unit_id()])
    ...
    for candidate in candidates.iter().take(CANDIDATE_BUDGET) {
        ...
        let terminal_rows = match search_session
            .catchments()
            .query_by_ids(&[resolved.authority().unit_id()])
        {
        ...
        let resolved_outlet_native = forward(crs, resolved.authority().resolved_coord());
    ...
    let (snap_id, weight, mainstem_status, distance_m, candidates_considered) =
        match resolved.authority().method() {
    ...
    let evidence = Evidence {
        input_coord: [
            resolved.authority().input_coord().lon,
            resolved.authority().input_coord().lat,
        ],
        resolved_coord: [
            resolved.authority().resolved_coord().lon,
            resolved.authority().resolved_coord().lat,
        ],
        resolved_terminal_id: resolved.authority().unit_id().get(),
        snap: SnapEvidence {
            method: "Snap",
        ...
        seam_search: SeamSearchEvidence {
            ...
            selected_resolved_terminal_id: resolved.authority().unit_id().get(),
        },
        terminal_windows,
        overlap: OverlapEvidence {
