...
`shed` reads a hydrofabric, a dataset that has already divided the landscape into unit catchments.
...
`shed` reads hydrofabrics in the HFX format, a folder of pre-built river-network files.
Because the landscape units and their connections are prepared ahead of time, delineation becomes a lookup and a merge.

## What shed does with your point

First, `shed` finds the unit catchment that contains your point.
...
Next, `shed` follows the recorded catchment connections upstream.
It gathers every unit catchment whose water drains toward the point.

Finally, `shed` merges those unit catchments into one polygon.
...
As one optional last refinement step, when a dataset includes a D8 flow-direction raster `shed` trims the outlet's own catchment to the exact point, and since the hosted GRIT dataset ships no raster `shed` keeps that catchment whole and still returns the full watershed.

This approach comes from Matthew Heberger's open-source `delineator` project, which inspired `shed`.
